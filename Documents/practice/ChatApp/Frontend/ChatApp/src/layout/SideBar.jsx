...
function SideBar({ toggle, setToggle, setReceiver, socket, socketInstanse }) {
  ...
  useEffect(() => {
    const handleLogin = (userId) => {
      setItems((prevItems) =>
        prevItems.map((elem) =>
          elem._id === userId ? { ...elem, loginState: "login" } : elem
        )
      );
    };
    ...
    const handleLogout = (id) => {
      setItems((prevItems) =>
        prevItems.map((elem) =>
          elem._id === id ? { ...elem, loginState: "" } : elem
        )
      );
    };
          ...
          {items.map((elem, i) =>
            elem.username !== uname ? (
              ...
              >
                ...
                />
                <div>
                  {elem.username} <span>{elem.loginState ? "Active" : ""}</span>
                </div>
              </div>
            ) : (
              ""
